...
def create_vocab(network):
    ...
    for node in network.graph.iter_nodes():
        if node.name not in word_vocab:
            ...
            word_vocab[node.name] = network.graph.node2id[node.name]
...
def write_vocab(word_vocab, vocab_file):
    with open(vocab_file,"w") as f:
        for i, (word, idx) in enumerate(word_vocab.items()):
            f.write("{}\t{}\n".format(word, idx))
            #f.write("{}\n\n".format(word))
    print("Write {} vocab to {}".format(i+1, vocab_file))
...
def prepare_batch_dgl(vocab, test_network, train_network, exist_relation=True):
    all_edges = []
    all_labels = []
    for edge in test_network.graph.iter_edges():
        ...
        if exist_relation:
            ...
            all_edges.append(np.array([src_id, tgt_id]))
