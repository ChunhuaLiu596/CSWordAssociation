...
def get_pos(s):
    ...
    """
    pos = "unkpos"
    if s.endswith("/n") or s.endswith("/a") or s.endswith("/v") or s.endswith("/r") or s.endswith("/s"):
        # pos = s[-1]
        pos= pos_dict.get(s[-2:], "unkpos")
        s = s[:-2]
    ...
    with open(conceptnet_path, 'r', encoding="utf8") as fin, \
        ...
        for line in tqdm(fin, total=num_lines):
            ...
            if toks[2].startswith('/c/en/') and toks[3].startswith('/c/en/'):
                ...
                head, head_pos = get_pos(toks[2])
                tail, tail_pos = get_pos(toks[3])
                head = head.split("/")[-1].lower()
                tail = tail.split("/")[-1].lower()
                ...
                if cur_weight>=weight:
                    fout.write('\t'.join([rel, head, tail, str(data["weight"]), head_pos, tail_pos]) + '\n')
                    count+=1
...
if __name__ == "__main__":
    ...
    debug = sys.argv[1]
    extract_english("ConceptNet.pos",
            conceptnet_path='data/cpnet/conceptnet-assertions-5.6.0.csv',
            output_csv_path='data/analysis/cpnet_17rel_pos/conceptnet.en.pos.csv',
            output_vocab_path = 'data/analysis/cpnet_17rel_pos/concept.txt',
            output_relation_path = 'data/analysis/cpnet_17rel_pos/relation.txt', debug=debug)
