...
parser.add_argument('config_path', default='paths.cfg', help='path for paths.cfg')
...
def create_embeddings_glove_from_vocab(config_path, kg_name, pooling="max", dim=100):

    print("Pooling: " + pooling)

    config = configparser.ConfigParser()
    #config.read("./embeddings/paths.cfg")
    config.read(config_path)

    with open(config["paths_{}".format(kg_name)]["concept_vocab"], "r", encoding="utf8") as f:
        concept_vocab = f.readlines()
    print("Loaded " + str(len(concept_vocab)) + " concepts")

    with open(config["paths_{}".format(kg_name)]["relation_vocab"], "r", encoding="utf8") as f:
        relation_vocab = f.readlines()
    print("Loaded " + str(len(relation_vocab)) + " relations")
    ...
    for i in tqdm(range(len(concept_vocab))):
        concept = concept_vocab[i].strip()

        concept_list = concept.split(" ")

        # counting the frequency (only used for the avg pooling)
        if concept not in concept_embeddings:
            concept_embeddings[concept] = np.zeros((dim,))

        if pooling == "max":
            concept_encoding = np.amax([glove_embeddings.get(word, np.zeros((dim,))) for word in concept_list], axis=0)

            concept_embeddings[concept] = concept_encoding

    for i in tqdm(range(len(relation_vocab))):
        rel = relation_vocab[i].strip()
    ...
    write_embeddings_npy(concept_embeddings, None, config["paths_{}".format(kg_name)]["concept_vec_npy_glove"] + "." + pooling,
                         config["paths_{}".format(kg_name)]["concept_vocab_glove"] + "." + pooling + ".txt")
    write_embeddings_npy(rel_embeddings, None, config["paths_{}".format(kg_name)]["relation_vec_npy_glove"] + "." + pooling,
                         config["paths_{}".format(kg_name)]["relation_vocab_glove"] + "." + pooling + ".txt")
...
def create_embeddings_glove(pooling="max", dim=100):
    ...
    for i in tqdm(range(len(triple_str_json))):
        ...
        if pooling == "avg":
            ...
            rel_embeddings[rel] = weighted_average(rel_embeddings[rel], rel_encoding, rel_embeddings_cnt[rel])
            # posy: content in rel_embeddings[rel] are zero? so it's just max/n?
...
if __name__ == "__main__":
    ...
    create_embeddings_glove_from_vocab(config_path=args.config_path,kg_name=args.kg_name)
