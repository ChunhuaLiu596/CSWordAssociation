...
def run():
    ...
    config.set_out_files(OUTPUT_PATH + "glove.TransH."+opt_method+".vec.json")

    print("Opt-method: %s" % opt_method)
    print("Pretrain: %d" % pretrain)
    config.init()
    print("set_model")
    config.set_model(models.TransH, args.kg_name)

    print("Begin training TransH")
...
def init_predict(hs, ts, rs):
    ...
    con.set_import_files("openke_data/embs/glove_initialized/glove.TransH.SGD.pt")
    con.init()
    ...
    con.init()
    con.set_model(models.TransH)
    #f = open("./openke_data/embs/glove_initialized/glove.TransH.SGD.vec.json", "r")
    f = open("./openke_data/swow/3rel_freq1/embs/glove_initialized/glove.TransH.SGD.vec.json","r")
