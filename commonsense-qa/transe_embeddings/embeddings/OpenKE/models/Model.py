...
class Model(object):
	...
	def __init__(self, config, kg_name):
		...
		if config.pretrain:
			...
			concept_init_embs = gconfig["paths_{}".format(kg_name)]["concept_vec_npy_glove"] + '.max.npy'
			relation_init_embs = gconfig["paths_{}".format(kg_name)]["relation_vec_npy_glove"] + '.max.npy'
			use_pretrain_relation = gconfig["paths_{}".format(kg_name)]["use_pretrain_rel_emb"] 

			with tf.name_scope("embedding"):

				self.initial_ent_embeds = np.load(concept_init_embs)
				if use_pretrain_relation:
					self.initial_rel_embeds = np.load(relation_init_embs)
				else:
					self.initial_rel_embeds = None
		'''revision ends'''
